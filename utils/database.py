...
class Database():
    def __init__(self, config):
        ...
        connection_string = "host={} dbname={} user={} password={} port={}".format(
                self.config.get("database_address"), self.config.get("database_name"), self.config.get("database_user"),
    ...
    def get_releases(self, distro=None):
        if not distro:
            self.c.execute("select distro, release from releases")
            return self.c.fetchall()
        else:
            self.c.execute("select release from releases WHERE distro=%s", (distro,))
            releases = self.c.fetchall()
            respond = []
    ...
    def insert_profiles(self, distro, release, target, subtarget, packages_default, profiles):
        ...
        self.c.execute("INSERT INTO packages_default VALUES (%s, %s, %s, %s, %s);", (distro, release, target, subtarget, packages_default))
    ...
    def get_subtargets(self, distro, release, target="%", subtarget="%"):
        ...
        self.c.execute("""SELECT target, subtarget, supported FROM subtargets
                    WHERE distro = %s and release = %s and target LIKE %s and subtarget LIKE %s;""",
                       (distro, release, target, subtarget))
        return self.c.fetchall()
    ...
    def get_supported_releases(self, distro):
        if distro == '': distro='%'
        sql = """select coalesce(array_to_json(array_agg(row_to_json(releases))), '[]') from (select distro, release from releases where distro LIKE %s order by id desc) as releases;"""
        self.c.execute(sql, distro)
        return self.c.fetchone()[0]
    ...
    def get_supported_models(self, search='', distro='', release=''):
        ...
        self.c.execute(sql, (search_like, distro, release))
