...
#            return None
    ...
    def get_supported_releases(self, distro):
        if distro == '': distro='%'
        sql = """select coalesce(array_to_json(array_agg(row_to_json(releases))), '[]') from (select distro, release from releases where distro LIKE ? order by id desc) as releases;"""
