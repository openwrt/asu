    ...
    def run(self):
        ...
        while True:
            ...
            if not outdated_subtarget:
                ...
                distro, version, target, subtarget = outdated_subtarget
                imagebuilder = ImageBuilder(distro, str(version), target, subtarget)
                ...
                if info:
                    self.db.insert_profiles(distro, version, target, subtarget, *info)
                else:
                    logging.error("could not receive profiles of %s/%s", target, subtarget)
                    self.db.subtarget_synced(distro, version, target, subtarget)
                    continue

                if os.path.exists(os.path.join(
                        self.config.get_folder("imagebuilder_folder"), distro,
                        version, target, subtarget, "target/linux", target,
                        "base-files/lib/upgrade/platform.sh")):
                    self.log.info("%s target is supported", target)
                    self.db.insert_supported(distro, version, target)
                else:
                ...
                if packages:
                    self.db.insert_packages_available(distro, version, target, subtarget, packages)
                else:
